...
if [[ $( gh release view "${RELEASE_VERSION}" --repo "${ASSETS_REPOSITORY}" 2>&1 ) =~ "release not found" ]]; then
  ...
  if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
    ...
    gh release create "${RELEASE_VERSION}" --repo "${ASSETS_REPOSITORY}" --title "${CLAD_VERSION}" --notes "${NOTES}"
  else
    gh release create "${RELEASE_VERSION}" --repo "${ASSETS_REPOSITORY}" --title "${CLAD_VERSION}" --generate-notes
    ...
    replace "s|CLAD_VERSION|${CLAD_VERSION}|g" release_notes.txt
