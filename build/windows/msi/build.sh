...
if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
  PRODUCT_NAME="Clad - Insiders"
  PRODUCT_CODE="CladInsiders"
...
if [[ -z "${1}" ]]; then
	OUTPUT_BASE_FILENAME="${EXE_NAME}-${VSCODE_ARCH}-${RELEASE_VERSION}"
else
	OUTPUT_BASE_FILENAME="${EXE_NAME}-${VSCODE_ARCH}-${1}-${RELEASE_VERSION}"
fi
